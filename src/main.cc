...
//  const vector<double> global_cuts = {5300., 0.0, 0.55, -0.12, 0.95}; //FV, u.r, itr, beta14_min, beta14_max
  const vector<double> prompt_cuts={0.0,0.9,1000.}; //energy, nhits_min, nhits_max
  const vector<double> delayed_cuts={0.0,0.9,1000.}; //energy, nhits_min, nhits_max
...
//  if (!exists(SCANDIR)) scan(global_cuts);
  ...
  event_select("./your_ntuple.root","output", "prompt", prompt_cuts);
